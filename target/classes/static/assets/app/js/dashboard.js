...
var Dashboard = function() {
    ...
    var profitShareA = function() {
        if ($('#m_chart_profit_share_2').length == 0) {
            return;
        }

        var chart = new Chartist.Pie('#m_chart_profit_share_2', {
            series: [{
                value: 30,
                className: 'custom',
                meta: {
    ...
    var supportTickets1 = function() {
        if ($('#m_chart_support_tickets_1').length == 0) {
            return;
        }

        Morris.Donut({
            element: 'm_chart_support_tickets_1',
            data: [
                {
                    label: "Avancé",
                    value: 30
    ...
    var demo8 = function() {
        ...
        for (var i = 0; i < series; i++) {
            data[i] = {
                label: "Specialité" + (i + 1),
                data: Math.floor(Math.random() * 100) + 1
            };
        }

        $.plot($("#spec_chart"), data, {
            series: {
                pie: {
                    show: true
                }
